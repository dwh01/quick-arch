...
#
#
# def enum_script_previews_from_directory_items(self, context):
...
#     directory = wm.qarch_asset_dir
#
#     # Get the preview collection (defined in register func).
#     pcoll = preview_collections["scripts"]
#
...
#
#         pcoll.script_previews = enum_items  # storage of list
#         pcoll.my_previews_dir = directory   # so we can update if it changes
#
#     if len(wm.script_filter) > 1:
#         filt = wm.script_filter.lower()
#         lst_enum = [e for e in pcol.script_previews if filt in e[0].lower()]
#         return lst_enum
#     return pcoll.script_previews
#
# def update_script_enum(self, context):
#     bpy.ops.qarch.select_img('INVOKE_DEFAULT', False)
#
# def register_wm_props():
#     # install session properties
#     path = pathlib.Path(__file__)  # qarch/ops/__init__.py
#     path = path.parent.parent / pathlib.Path("assets")
#
#     WindowManager.qarch_asset_dir = StringProperty(name="Catalog Path", subtype='DIR_PATH', default=str(path))
#     WindowManager.script_previews = EnumProperty(items=enum_script_previews_from_directory_items)  #, update=update_script_enum)
#     WindowManager.script_filter = StringProperty(name="Filter Scripts")
#
#     pcoll = bpy.utils.previews.new()
#     pcoll.my_previews_dir = ""  # last directory loaded
#     pcoll.script_previews = ()  # storage for enum list
#
#     preview_collections["scripts"] = pcoll

# def unregister_wm_props():
#     del WindowManager.qarch_asset_dir
#
